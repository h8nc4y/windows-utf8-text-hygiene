...
2. **`TerminatorExpectedAtEndOfString`。** 該当バイトで終わる文字列
   ...
   がありません」になる。
3. **`UnexpectedToken`。** 同じ機序が構造トークンを食い、囲んでいる構文
   ごと壊れる — 実測では `param(...)` の既定値文字列が終端を失った結果、
   function の閉じ波括弧で `Unexpected token '}'` になった。パーサの
   エラー ID は構文の組み方で変わる。不変なのは機序と「囲んでいる定義が
   壊れる」という帰結。
